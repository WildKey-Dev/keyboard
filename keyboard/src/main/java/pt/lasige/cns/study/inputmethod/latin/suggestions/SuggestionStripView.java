...
package pt.lasige.cns.study.inputmethod.latin.suggestions;
...
import pt.lasige.cns.study.inputmethod.keyboard.KeyboardTheme;
import pt.lasige.cns.study.inputmethod.keyboard.MainKeyboardView;
import pt.lasige.cns.study.inputmethod.keyboard.MoreKeysPanel;
import pt.lasige.cns.study.inputmethod.latin.AudioAndHapticFeedbackManager;
...
import pt.lasige.cns.study.inputmethod.latin.suggestions.MoreSuggestionsView.MoreSuggestionsListener;
...
import pt.lasige.cns.study.inputmethod.study.PromptLauncherActivity;
...
//        mVoiceKey.setVisibility(currentSettingsValues.mShowsVoiceInputKey ? VISIBLE : INVISIBLE);
    ...
    public void updateLogButtonVisibility() {
        if(MetricsController.getInstance().mode == StudyConstants.IMPLICIT_MODE){
            if(LoggerController.getInstance().isLog())
                if(KeyboardTheme.getKeyboardTheme(getContext()).mThemeId == 0 ||
                        KeyboardTheme.getKeyboardTheme(getContext()).mThemeId == 2 ||
                        KeyboardTheme.getKeyboardTheme(getContext()).mThemeId == 4)
                    mKillSwitch.setImageResource(R.drawable.ic_baseline_lock_open_24_white);
                else
                    mKillSwitch.setImageResource(R.drawable.ic_baseline_lock_open_24);
            else
            if(KeyboardTheme.getKeyboardTheme(getContext()).mThemeId == 0 ||
                    KeyboardTheme.getKeyboardTheme(getContext()).mThemeId == 2 ||
                    KeyboardTheme.getKeyboardTheme(getContext()).mThemeId == 4)
                mKillSwitch.setImageResource(R.drawable.ic_baseline_lock_24_white);
            else
                mKillSwitch.setImageResource(R.drawable.ic_baseline_lock_24);

            mKillSwitch.setVisibility(View.VISIBLE);
        }else{
            mKillSwitch.setVisibility(View.INVISIBLE);
        }
    }
    ...
    public void onClick(final View view) {
        ...
        if (view == mVoiceKey) {
            ...
            LoggerController.getInstance().getLogger().incVoiceInput();

            return;
        }
        if(view == mKillSwitch){

            LoggerController.getInstance().setLog(getContext(), !LoggerController.getInstance().isLog());
            ...
            if(LoggerController.getInstance().isLog()) {
                ...
                Toast.makeText(getContext(), R.string.log_mode_activated, Toast.LENGTH_SHORT).show();
            }else {
                if (KeyboardTheme.getKeyboardTheme(getContext()).mThemeId == 0 ||
                        KeyboardTheme.getKeyboardTheme(getContext()).mThemeId == 2 ||
                        KeyboardTheme.getKeyboardTheme(getContext()).mThemeId == 4)
        ...
        if (tag instanceof Integer) {
            ...
            LoggerController.getInstance().getLogger().addSuggestion(wordInfo.mWord, true, true);
        }
    }
