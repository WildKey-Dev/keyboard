...
import pt.lasige.ideafast.study.inputmethod.logger.data.KeyEventData;
import pt.lasige.ideafast.study.inputmethod.logger.data.MotionEventData;
...
import pt.lasige.ideafast.study.inputmethod.metrics.textentry.datastructures.Error;
...
import static pt.lasige.ideafast.study.inputmethod.latin.common.Constants.CODE_ENTER;
import static pt.lasige.ideafast.study.inputmethod.latin.common.Constants.CODE_SPACE;
...
//        if(character.equals("<<") && lastInputWasDeletable){
    ...
    public void writeToBuffer(Key key, int x, int y, long eventTime){
        ...
        if(key.getCode() == CODE_ENTER && MetricsController.getInstance().mode != StudyConstants.IMPLICIT_MODE )
            return;
...
//            if(!isCursorOnEnd) {
    ...
    public void addCursorChange(CursorChange cursorChange){
        ...
        if(!wasEditTextEmpty && MetricsController.getInstance().mode == StudyConstants.IMPLICIT_MODE) {
