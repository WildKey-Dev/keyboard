...
public class MetricsController {
    ...
    private void calculateAll(final Logger log, final String targetPhrase, String questionID, String studyID, int phraseNumber, boolean removeSuggestionsFromInputStream){
        ...
        try {
            ...
            DataBaseFacade.getInstance().write("touch-offset-target", "toBeAdded", "/users/"+ DataBaseFacade.getInstance().getFbUserID()+"/completedTasks/"+studyID+"/"+questionID+"/phrases/"+phraseNumber+"/");
            DataBaseFacade.getInstance().write("touch-offset-error", "toBeAdded", "/users/"+ DataBaseFacade.getInstance().getFbUserID()+"/completedTasks/"+studyID+"/"+questionID+"/phrases/"+phraseNumber+"/");
    ...
    public void onKeyboardHide() {
        ...
        if(mode == StudyConstants.IMPLICIT_MODE){
            ...
            if(log.wasEditTextEmpty()){
                runMetricCalculation(log, null, String.valueOf(System.currentTimeMillis()), "implicit_mode", 0);
            }else {
                DataBaseFacade.getInstance().write("discarded", "edit box was not empty", "/users/"+ DataBaseFacade.getInstance().getFbUserID()+"/completedTasks/implicit_mode/"+String.valueOf(System.currentTimeMillis())+"/phrases/"+0+"/");
            }
